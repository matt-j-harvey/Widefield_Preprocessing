...
class skeleton_assignment_window(QWidget):

    def __init__(self, parent=None):
        super(skeleton_assignment_window,  self).__init__(parent)
        ...
        self.finished_mapping_button.clicked.connect(self.finish_mapping)
        ...
        self.layout.addWidget(self.map_button,                  8, 0, 1, 2)
        self.layout.addWidget(self.finished_mapping_button,     9, 0, 1, 2)
    ...
    def load_max_projection(self):
        ...
        if self.base_directory == None:
            ...
            max_projection_file_location = os.path.join(self.base_directory, "max_projection.npy")

        print(max_projection_file_location)

        # Check Max Projection
        self.max_projection = np.load(max_projection_file_location, allow_pickle=True)
        self.max_projection  = np.divide(self.max_projection, np.max(self.max_projection ))
        self.max_projection  = np.expand_dims(self.max_projection , axis=2)
        self.max_projection = np.repeat(a=self.max_projection , repeats=3, axis=2)
...
def align_skeleton_main():

    window = skeleton_assignment_window()

...
class session_matching_window(QWidget):
    ...
    def select_template_session(self):
        ...
        # Load Max Projection
        max_projection = np.load(self.template_directory + "/max_projection.npy")
        self.template_max_projection = max_projection

        # Load Skeleton
        skeleton_array = np.load(self.template_directory + "/Skeleton.npy")
        self.template_skeleton_image = skeleton_array

        # Load Retinotopy Contours
        retinotopy_array = np.load(self.template_directory + "/Registered_Contour_Array.npy")
        self.template_functional_image = retinotopy_array

        # Get New Directory + Session Name
        new_session_directory_split = self.template_directory.split("/")
    ...
    def select_matching_session(self):
        ...
        # Get Template Directory
        if self.matching_directory == None:
        ...
        # Load Max Projection
        max_projection = np.load(self.matching_directory + "/max_projection.npy", allow_pickle=True)
        self.matching_max_projection = max_projection

        # Load Skeleton
        skeleton_array = np.load(self.matching_directory + "/Skeleton.npy", allow_pickle=True)
        self.matching_skeleton_image = skeleton_array

        # Load Retinotopy Contours
        if os.path.isfile(self.matching_directory + "/Registered_Contour_Array.npy"):
            retinotopy_array = np.load(self.matching_directory + "/Registered_Contour_Array.npy")
        else:
        ...
        # Get New Directory + Session Name
        new_session_directory_split = self.matching_directory.split("/")
        new_session_name = new_session_directory_split[-2] + "_" + new_session_directory_split[-1]
...
def align_widefield_sessions_main():
